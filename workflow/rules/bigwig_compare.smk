...
if BWC_ENABLED:
    ...
    rule pool_bigwigs:
        ...
        input:
            ...
            helper=workflow.source_path("scripts/bigwig_math.py")
        output:
            bw=temp(f"{OUTDIR}/tmp/bigwig_compare/{{group}}.{{track}}.{{pool}}.pooled.bw")
        log:
            f"{OUTDIR}/logs/bigwig_compare/{{group}}.{{track}}.{{pool}}.pool.log"
        conda:
            "envs/py_signal.yaml"
        script:
            "scripts/average_bigwigs.py"

    # Sparse interval math preserves exact per-base values without scanning zeros.
    if int(BWC_CFG.get("binSize", 1)) == 1 and BWC_OPERATION in {"log2", "ratio", "subtract", "add"}:
        ...
        rule bigwig_compare_log2:
            ...
            output:
                bw=f"{OUTDIR}/bigwig/compare/{{group}}.{{track}}.{BWC_OPERATION}.bw"
            log:
                f"{OUTDIR}/logs/bigwig_compare/{{group}}.{{track}}.{BWC_OPERATION}.log"
